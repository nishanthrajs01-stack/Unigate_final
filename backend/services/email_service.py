"""
Email Microservice — UCMS
Async SMTP email sending with HTML templates.
"""
import os
import asyncio
import smtplib
...
def _get_base_template(content: str, title: str = "Unigate Consultancy") -> str:
                        ...
                        <tr>
                            <td style="background-color:#f8fafc;padding:25px 40px;border-top:1px solid #e2e8f0;">
                                <p style="color:#94a3b8;font-size:12px;margin:0;text-align:center;">
                                    &copy; 2024 Unigate Consultancy. All rights reserved.<br>
...
def get_welcome_email(student_name: str) -> tuple[str, str]:
    """Return (subject, html) for welcome email."""
    subject = "Welcome to Unigate Consultancy — Your Journey Begins!"
    content = f"""
        <h2 style="color:#0f172a;margin:0 0 20px;">Welcome, {student_name}! 🎓</h2>
        ...
        <div style="text-align:center;margin:30px 0;">
            <a href="{APP_URL}/student"
               style="background-color:#0f172a;color:#ffffff;padding:14px 32px;
    ...
    """
    return subject, _get_base_template(content, subject)


def get_status_change_email(student_name: str, new_status: str, notes: str = "") -> tuple[str, str]:
    """Return (subject, html) for application status change."""
    status_colors = {
        "pending": "#f59e0b",
        "under_review": "#3b82f6",
        "verified": "#10b981",
    ...
    subject = f"Application Update — Status: {display_status}"
    content = f"""
        <h2 style="color:#0f172a;margin:0 0 20px;">Application Status Update</h2>
        <p style="color:#475569;font-size:15px;line-height:1.7;">
            Dear {student_name}, your application status has been updated:
        </p>
        <div style="text-align:center;margin:25px 0;padding:20px;background-color:#f8fafc;border-radius:10px;border-left:4px solid {color};">
            ...
            <p style="color:{color};font-size:22px;font-weight:700;margin:0;">{display_status}</p>
        </div>
        {"<p style='color:#475569;font-size:14px;background:#fffbeb;padding:12px 16px;border-radius:6px;border-left:3px solid #f59e0b;'><strong>Note:</strong> " + notes + "</p>" if notes else ""}
        <div style="text-align:center;margin:30px 0;">
            <a href="{APP_URL}/student"
               style="background-color:#0f172a;color:#ffffff;padding:14px 32px;
                      border-radius:8px;text-decoration:none;font-weight:600;
...
def get_admission_email(student_name: str, college_name: str, program: str) -> tuple[str, str]:
    """Return (subject, html) for admission confirmation."""
    subject = f"🎉 Congratulations! Admission Confirmed — {college_name}"
    content = f"""
        <div style="text-align:center;margin-bottom:25px;">
            <span style="font-size:48px;">🎓</span>
        </div>
        <h2 style="color:#0f172a;margin:0 0 20px;text-align:center;">Congratulations, {student_name}!</h2>
        ...
        <div style="text-align:center;margin:30px 0;">
            <a href="{APP_URL}/student"
               style="background:linear-gradient(135deg,#0f172a,#1e293b);color:#ffffff;padding:14px 32px;
                      border-radius:8px;text-decoration:none;font-weight:600;
                      font-size:15px;display:inline-block;box-shadow:0 4px 12px rgba(15,23,42,0.3);">
...
async def send_email(to_email: str, subject: str, html_content: str) -> dict:
    ...
    msg["From"] = f"Unigate Consultancy <{SMTP_USER}>"
    msg["To"] = to_email
    msg["Subject"] = subject
